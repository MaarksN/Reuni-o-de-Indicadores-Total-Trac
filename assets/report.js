(() => {
  ...
  // Uma venda != um cliente: um cliente pode ter várias linhas (dispositivos) na mesma coorte.
  const uniqueClients = (items) => new Set(items.map((item) => item.cliente)).size;
  ...
  const clientesNovosUnicos = uniqueClients(clientesNovos);
  const clientesExistentesUnicos = uniqueClients(clientesExistentes);
  ...
  // Ordinal calculado a partir do dado real, nunca escrito à mão — evita a frase ficar
  // desatualizada se um mês novo entrar na série.
  const monthsRankedByRealizado = [...data.summary2026].sort((a, b) => b.realizado - a.realizado).map((item) => item.month);
  const referenceMonthRank = monthsRankedByRealizado.indexOf(referenceMonth) + 1;
  const ordinalMelhorMes = { 1: 'melhor', 2: 'segundo melhor', 3: 'terceiro melhor', 4: 'quarto melhor', 5: 'quinto melhor', 6: 'sexto melhor', 7: 'sétimo melhor' }[referenceMonthRank] || `${referenceMonthRank}º melhor`;
  ...
  const health = {
    ...
    'Vendas · Clientes Novos': Math.min(100, pctNovos / metaNovosClientes * 100),
  ...
  const healthDetailRows = [
    ...
    ['Vendas · Clientes Novos', `${pct(pctNovos)} das vendas em clientes novos (${clientesNovosUnicos} cliente(s) único(s))`, `Meta de gestão: ${metaNovosClientes}% de vendas em clientes novos`, `mín(100, ${one.format(pctNovos)}/${metaNovosClientes}×100) = ${int.format(health['Vendas · Clientes Novos'])}`],
  ...
  const vendedorCards = vendedorRows.map(([name]) => {
    ...
    const novosItems = items.filter((item) => item.tipoVenda === 'Novo');
    const novosUnicos = uniqueClients(novosItems);
    return `<article class="card card-pad"><h3>${esc(name)}</h3><p><b>${items.length}</b> vendas · <b>${brl.format(sum(items, 'mensalidade'))}</b> mensalidade · <b>${brl.format(sum(items, 'adesao'))}</b> adesão</p><p>${novosUnicos} cliente(s) novo(s) em ${novosItems.length} venda(s) · ${pct(novosItems.length / items.length * 100)} de aquisição</p></article>`;
  }).join('');
  ...
  pages.push(page(14, 'Clientes', 'Novos Clientes × Carteira Existente', `${clientesNovosUnicos} clientes novos em ${referenceMonth.toLowerCase()} (${clientesNovos.length} venda(s), ${pct(pctNovos)} do total); o restante veio de upsell nos ${clientesExistentesUnicos} clientes da carteira já ativa.`, `<div class="grid grid-4">${kpi('Clientes novos', int.format(clientesNovosUnicos), `${clientesNovos.length} venda(s) · ${pct(pctNovos)} das vendas`, 'julho-novo', 'accent-card')}${kpi('Carteira existente', int.format(clientesExistentesUnicos), `${clientesExistentes.length} venda(s) · ${pct(100 - pctNovos)} das vendas`, 'julho-existente')}${kpi('Mensalidade em clientes novos', brl.format(sum(clientesNovos, 'mensalidade')), pct(sum(clientesNovos, 'mensalidade') / mrrNovo * 100) + ' da mensalidade nova', 'julho-novo')}${kpi('Mensalidade em carteira existente', brl.format(sum(clientesExistentes, 'mensalidade')), pct(sum(clientesExistentes, 'mensalidade') / mrrNovo * 100) + ' da mensalidade nova', 'julho-existente')}</div>${sectionTitle('Comparativo do mês · clientes únicos')}<div class="card card-pad">${bars([['Clientes novos', clientesNovosUnicos], ['Carteira existente', clientesExistentesUnicos]])}</div>${sectionTitle('Clientes novos de ' + referenceMonth)}<div class="company-grid">${clienteRows.filter((row) => row.novo).map((row) => `<div class="company-card"><strong>${esc(row.name)}</strong><span>${brl.format(row.total)} total · ${row.count} venda(s)</span></div>`).join('') || '<p class="muted">Nenhum cliente novo identificado no mês.</p>'}</div>${source('campo tipoVenda de cada venda de ' + referenceMonth)}`));
